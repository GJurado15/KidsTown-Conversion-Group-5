...
and its replacement: a static, framework-free site deployable on GitHub
Pages, now live at the repo root.

**Live site:** https://gjurado15.github.io/KidsTown-Conversion-Group-5/

**Legacy site (as imported, non-interactive):**
https://gjurado15.github.io/KidsTown-Conversion-Group-5/legacy/
...
The replacement lives at the repo root (all 9 rooms migrated, cut over from
its original staging location under `site/`) and reuses the legacy
`graphics/` folder directly (relative paths) rather than duplicating image
assets. It is plain HTML/CSS/JS — no framework, no build step. The original
Perl/CGI source stays in place at `cgi-bin/`, `scripts/`, `data/` for
reference, and a live copy of the legacy demo page is preserved at
`legacy/index.html`.

- `index.html`, `about.html`, `participants.html`, `help.html` — the
  Home room, ported from `scripts/home/*.pl`.
- `citypark/` — the City Park interactive story, ported from
...
- `museum/` — the Museum room: lobby, Color Exhibition, a 6-page
...
- `zoo/` — the Zoo room: lobby, world map, four region encyclopedia
...
- `library/` — the Library room: a U.S. map, 7 region pages, and
...
- `cityhall/` — the City Hall room: two branching detective stories
...
- `assets/nav.js` — shared nav bar, used by every room.
- `toystore/` — the ToyStore room: 7 riddles (`riddle.html?n=`) and 5
...
- `school/` — the School room: Word Fun (a hangman-style guessing
...
- `township/` — the TownShip room: `wonders/` (4 "Wonders of the
...
| 2026-09-10 | Added the TownShip section under `site/township/`: 4 Wonders-of-the-World quiz pages and a 6-round Country Shape Matching Game (18 countries). **All 9 rooms are now migrated.** Verified every page, quiz, and referenced image (out of 124 graphics) in a headless browser. |
| 2026-09-10 | Full cross-room QA pass: an automated crawler resolved all 512 literal `href`/`src` references site-wide (zero broken) plus every dynamic query-param ID (Zoo regions, Library state/activity, Museum tours, Farm cross-links, TownShip countries/rounds, City Hall's story graph) against its data file — zero dangling references. Confirmed no leftover "(coming soon)" entries anywhere. |
| 2026-09-10 | **Cut root over to the replacement.** Moved everything from `site/` up to the repo root (one `graphics/` path level removed from every file to match the new depth, mechanically verified with the same link-crawl before and after) and moved the legacy demo to `legacy/index.html`. The site root now serves the finished static site directly; the original Perl/CGI source is untouched at `cgi-bin/`, `scripts/`, `data/`. Re-ran the full QA crawl and a console-error scan against the new layout — clean. |
